...
const FinanceStep: React.FC<StepProps> = ({ onNext, onBack }) => {
  ...
  const [salary, setSalary] = useState(money.monthlyIncome?.toString() || '');
  const [currency, setCurrency] = useState(money.currency || 'AED');

  const handleNext = () => {
    setMoney({
      monthlyIncome: parseFloat(salary) || 0,
      currency,
    });
    onNext();
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-3">
        ...
        <Button variant="primary" fullWidth onClick={handleNext} icon={<ArrowRight className="w-4 h-4" />} iconPosition="right">
          Continue
        </Button>
      </div>
    </motion.div>
...
const HealthStep: React.FC<StepProps> = ({ onNext, onBack }) => {
  ...
  const [calories, setCalories] = useState(health.dailyCalorieGoal?.toString() || '2000');
  const [protein, setProtein] = useState(health.dailyProteinGoal?.toString() || '150');
  const [steps, setSteps] = useState(health.dailyStepGoal?.toString() || '10000');

  const handleNext = () => {
    setHealth({
      dailyCalorieGoal: parseInt(calories) || 2000,
      dailyProteinGoal: parseInt(protein) || 150,
      dailyStepGoal: parseInt(steps) || 10000,
    });
    onNext();
  };
...
const GardenIntroStep: React.FC<{ onFinish: () => void; onBack: () => void }> = ({ onFinish, onBack }) => (
  ...
  >
    ...
    <h2 className="text-2xl font-display font-bold text-earth-900 mb-3">
      Your Garden Awaits
    </h2>
    <p className="text-earth-600 mb-2 max-w-sm">
      Every workout, healthy meal, and smart money move earns
      sunlight for your garden. Watch your plants grow as you
      build better habits.
    </p>
    <p className="text-earth-500 text-sm mb-10 max-w-sm">
      You'll start with a small seed. Nurture it daily and watch it bloom.
    </p>
    <div className="flex gap-3">
      <Button variant="ghost" onClick={onBack} icon={<ArrowLeft className="w-4 h-4" />}>
        Back
      </Button>
      <Button variant="warm" size="lg" onClick={onFinish} icon={<Check className="w-5 h-5" />} iconPosition="right">
        Start Growing
      </Button>
    </div>
  </motion.div>
    ...
    {Array.from({ length: total }).map((_, i) => (
      <motion.div
        ...
        animate={{
          ...
          backgroundColor: i === current ? 'var(--color-sage-500)' : i < current ? 'var(--color-sage-300)' : 'var(--color-cream-300)',
        }}
