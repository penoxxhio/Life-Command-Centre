...
const getCategoryEmoji = (key: string) => EXPENSE_CATEGORIES.find((c) => c.key === key)?.emoji ?? '📦';
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <Card className="p-8 text-center">
          <p className="text-earth-500">No transactions yet</p>
        </Card>
      ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-1">
                          {t.type === 'expense' ? (
                            ...
                            <ArrowUpRight className="w-3.5 h-3.5 text-sage-500" />
                          )}
                          <span className={`text-sm font-bold ${t.type === 'expense' ? 'text-rose-600' : 'text-sage-600'}`}>
