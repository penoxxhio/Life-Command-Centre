...
export const DebtTracker: React.FC = () => {
  ...
  const handleDelete = () => {
    if (!deleteId) return;
    setMoney({ debts: debts.filter((d) => d.id !== deleteId) });
    setDeleteId(null);
  };
    ...
    <>
        ...
        <Card className="p-5 mb-4">
          <div className="flex justify-between text-sm mb-2">
            ...
            <span className="text-earth-500">
              {currency} {totalPaid.toLocaleString()} / {currency} {totalDebt.toLocaleString()}
            </span>
          </div>
          <ProgressBar value={totalPaid} max={totalDebt || 1} color="sage" size="lg" />
        </Card>
      )}
      ...
      {debts.length === 0 ? (
        <Card className="p-8 text-center">
          <Target className="w-10 h-10 text-earth-300 mx-auto mb-2" />
          <p className="text-sm text-earth-500">No debts tracked. That's great!</p>
        </Card>
      ) : (
              ...
              <motion.div key={debt.id} layout>
                <Card className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <p className="font-medium text-earth-900">{debt.name}</p>
                      <p className="text-xs text-earth-500">{pct}% paid off</p>
                    </div>
                    <button onClick={() => setDeleteId(debt.id)} className="text-earth-400 hover:text-rose-500 p-1">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                  <ProgressBar value={debt.paidAmount} max={debt.totalAmount || 1} color={pct >= 75 ? 'sage' : 'amber'} size="md" />
      ...
      <ConfirmDialog
        ...
        title="Delete Debt"
        message="Are you sure you want to remove this debt?"
        confirmLabel="Delete"
        variant="danger"
      />
    </>
