    ...
    <div className="space-y-4">
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className={`text-sm font-bold ${savingsRate >= 20 ? 'text-sage-600' : savingsRate >= 0 ? 'text-amber-600' : 'text-rose-600'}`}>
            {savingsRate}% saved
          </span>
        </div>
        <ProgressBar value={spent} max={income || 1} color={spent > income ? 'terracotta' : 'sage'} size="lg" showLabel />
          ...
          <div className="space-y-3">
            {categorySpending.map((cat) => (
              <div key={cat.key}>
                ...
                <ProgressBar value={cat.spent} max={spent || 1} color="amber" size="sm" />
              </div>
            ))}
          </div>
        </Card>
